...
log = logging.getLogger("Destiny2-Ultimate-Trainer-2026-extreme.TrainerCore")

class TrainerCore:
    def __init__(self, config: Config):
        self.config = config
        self.memory = MemoryAccess(config.process_name)
        self.running = False
    ...
    def _update_discord(self):
        ...
        try:
            discordrpc.update_presence(state=", ".join(active) if active else "Idle",
                                       details="Destiny2-Ultimate-Trainer-2026-extreme", large_image="logo", large_text="v1.3.0")
        except:
            pass
